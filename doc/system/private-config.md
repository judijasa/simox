...
port numbers — no secrets). `etc/hosts` is git-ignored but is a local
convenience mapping, not injected by `fetch-private-data`. Credentials
(`<ACCOUNT>_PASSWORD`) never live in any repo — `bin/gen-service-users` writes
them into the git-ignored, generated `etc/reuter.ini`.

## Usage

Copy `.private-source.example` to `.private-source` and point it at a checkout
of the private repo (tracked files: `machines.ini`, `team.ini`).
