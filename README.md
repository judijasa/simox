...
Create the dev connection file and the `simo` database (both git-ignored,
generated by `ema init db`):
```bash
ema init db simo
```

`ema` is a MariaDB package manager. Apply the schema packages from the root
package (the database name is explicit — the section header IS the database):
```bash
ema init tables simo-C196A24801D24B16 simo
```
