#!/usr/bin/env bash
# simox deploy wrapper — consumer entrypoint that wraps the framework
# `deploy` CLI (vendor/bin/deploy).
#
# The framework `deploy` is a closed operation: it swaps the repo, copies the
...
# runs the consumer post-deploy step (bin/deploy/post-nix.sh) on each host.
#
# Usage (from the repo root, inside `nix develop`):
#   bin/deploy-wrapper.sh                 # every [prod] host
#   bin/deploy-wrapper.sh <host>          # a single prod host (in [prod])
#   bin/deploy-wrapper.sh --init [host]   # + one-time provisioning
set -euo pipefail

# Run from the repo root (vendor/bin/deploy and etc/* are relative to it).
...
read_prod_roster() {
  php -r '
    ...
    if (!$cnf || !isset($cnf["prod"])) {
      fwrite(STDERR, "deploy-wrapper: no [prod] section in etc/machines.ini\n");
      exit(1);
    }
...
post_deploy_one() {
  local host="$1"
  ssh "root@$host" "cd '$DEPLOY_TARGET_DIR' && bin/deploy/post-nix.sh"
}
