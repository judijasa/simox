...
#   - MYSQL_*: derived from the repo root (pure path arithmetic on $PWD);
#     consumed by the dev shell hook (flake.nix) and the Makefile;
...
#     not exist).
# PROD_USER + DEPLOY_* are project-static and live in the committed
# etc/deploy.conf (framework `deploy` CLI contract), not in .env.
